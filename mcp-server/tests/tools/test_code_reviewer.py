...
class TestCodeReviewer:
    ...
    def test_review_code_with_valid_task(self, temp_dir, monkeypatch, workspace_manager, sample_project_dir):
        """测试审查有效任务的代码。"""
        # Arrange
        workspace_id = create_test_workspace(
            workspace_manager,
            sample_project_dir,
            requirement_name="用户认证"
        )
    ...
    def test_review_code_returns_review_result(self, temp_dir, monkeypatch, workspace_manager, sample_project_dir):
        """测试返回审查结果。"""
        # Arrange
        workspace_id = create_test_workspace(
            workspace_manager,
    ...
    def test_review_code_with_todo_in_file(self, temp_dir, monkeypatch, workspace_manager, sample_project_dir):
        """测试审查包含 TODO 的代码文件。"""
        # Arrange
        workspace_id = create_test_workspace(
            workspace_manager,
        ...
        # Assert
        assert result["success"] is True
        assert "TODO" in result["review_report"] or "todo" in result["review_report"].lower()
    
    def test_review_code_with_nonexistent_file(self, temp_dir, monkeypatch, workspace_manager, sample_project_dir):
        """测试审查不存在的代码文件。"""
        # Arrange
        workspace_id = create_test_workspace(
            workspace_manager,
        ...
        # Assert
        assert result["success"] is True
        assert "不存在" in result["review_report"] or "not found" in result["review_report"].lower()
    
    def test_review_code_with_short_file(self, temp_dir, monkeypatch, workspace_manager, sample_project_dir):
        """测试审查内容过短的文件。"""
        # Arrange
        workspace_id = create_test_workspace(
            workspace_manager,
        ...
        # Assert
        assert result["success"] is True
        assert "过短" in result["review_report"] or "short" in result["review_report"].lower()
    
    def test_review_code_with_read_error(self, temp_dir, monkeypatch, workspace_manager, sample_project_dir):
        """测试处理文件读取错误的情况。"""
        # Arrange
        workspace_id = create_test_workspace(
            workspace_manager,
        ...
        try:
            ...
            assert result["success"] is True
            # 应该包含错误信息
            assert "读取失败" in result["review_report"] or "error" in result["review_report"].lower()
        finally:
            # 恢复文件权限以便清理
            if os.name != 'nt':
