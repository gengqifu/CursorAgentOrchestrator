...
```
Cursor CLI → MCP Server (中央编排服务) → generate_prd 工具 → 项目代码仓库
```

### 步骤 0：确保工作区存在

在调用本 Skill 之前，应当已经通过 MCP Server 创建工作区：
...
### 步骤 2：通过 MCP Server 调用工具

**Agent 应该通过 MCP Server 调用 `generate_prd` 工具**：
...
**在 Cursor IDE 中使用**：
```
@agent-orchestrator generate_prd workspace_id="req-20240101-120000-user-auth" requirement_url="/path/to/requirement.md"
```
...
> - 核心实现在 `mcp-server/src/tools/prd_generator.py`
